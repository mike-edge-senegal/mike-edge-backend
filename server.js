/**
 * 🏆 PROJET MIKE EDGE - SERVER.JS (V11.11 GOLD MASTER - PHASE 1 SCELLÉ)
 * -------------------------------------------------------------------
 * Statut : SCELLÉ & CONSOLIDÉ (Parrainage Temps Phase 1 : 3 pour 1)
 * Certifications : Distinct-Referral (+30d), Admin-Key UTF8, Zero-Leak, Zero-Wildcard-CORS
 * -------------------------------------------------------------------
 */
...
// ==========================================
// 4. WEBHOOK PAIEMENT (PARRAINAGE PHASE 1 : DISTINCTS)
// ==========================================
...
app.post('/api/v1/payments/webhook', async (req, res) => {
    ...
    try {
        ...
        // A. Idempotence : Vérification de doublon
        ...
        // B. Activation de l'utilisateur payeur (+30 jours)
        await client.query(`
            UPDATE users 
            SET status = 'ACTIVE',
        ...
        if (referrerId) {
            ...
            if (totalDistinctPaidReferrals > 0 && totalDistinctPaidReferrals % 3 === 0 && totalDistinctPaidReferrals <= 9) {
                await client.query(`
                    ...
                    WHERE id = $1
                `, [referrerId]);
                console.log(`🎁 Récompense parrainage Phase 1 : 3 amis distincts ont payé ! +30 jours offerts pour le parrain ID ${referrerId}`);
            }
        }

        await client.query('COMMIT');
        res.json({ success: true, message: 'Compte activé et parrainage Phase 1 vérifié.' });
    } catch (err) {
...
const server = app.listen(PORT, () => {
    console.log(`🚀 SERVEUR MIKE EDGE V11.11 GOLD MASTER PHASE 1 SCELLÉ | PORT: ${PORT}`);
});
