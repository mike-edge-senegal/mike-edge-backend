...
async function getLeagueIdStrict(client, leagueName) { 
    ...
    const result = await client.query(`SELECT id FROM leagues WHERE LOWER(REGEXP_REPLACE(unaccent(name), '[^a-zA-Z0-9]', '', 'g')) = $1`, [normalized]); 
...
async function getTeamIdStrict(client, teamName, leagueId) { 
    ...
    let result = await client.query(`SELECT id FROM teams WHERE LOWER(REGEXP_REPLACE(unaccent(name), '[^a-zA-Z0-9]', '', 'g')) = $1 AND league_id = $2`, [normalized, leagueId]); 
    if (result.rows.length === 0) { 
        result = await client.query(`SELECT id FROM teams WHERE LOWER(REGEXP_REPLACE(unaccent(name), '[^a-zA-Z0-9]', '', 'g')) = $1 LIMIT 1`, [normalized]); 
        if (result.rows.length > 0) console.warn(`⚠️ Fallback international utilisé pour l'équipe: ${teamName}`); 
    } 
...
function buildRobustIsoDatetime(dateStr, timeStr) { 
    ...
    if (!Number.isInteger(year) || !Number.isInteger(month) || !Number.isInteger(day) || month < 1 || month > 12 || day < 1 || day > 31) throw new Error('ERR_INVALID_DATE_FORMAT'); 
    ...
    if (date.getUTCFullYear() !== year || date.getUTCMonth() !== month - 1 || date.getUTCDate() !== day || date.getUTCHours() !== hour || date.getUTCMinutes() !== minute) throw new Error('ERR_INVALID_CALENDAR_DATE'); 
    return date; 
}

async function insertBetRow(client, matchId, bet) { 
    const isBanned = bet.section_type === 'BANNED'; 
    await client.query(`INSERT INTO bets (match_id, section_type, rank_in_section, market_name, odds, probability_pct, robustness_pct, confidence_stars, comment) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [matchId, bet.section_type, bet.rank_in_section, bet.market_name, isBanned ? null : bet.odds, bet.probability_pct ?? null, bet.robustness_pct ?? null, isBanned ? null : bet.confidence_stars, bet.comment || null]); 
}
...
async function savePublicationTransaction(parsedData, targetPublicationId = null, userId = null) { 
    ...
    if (!validation.is_valid) return { success: false, code: 'ERR_VALIDATION_FAILED', errors: validation.errors, warnings: validation.warnings }; 
    try { 
        ...
        if (!publicationId) { 
            ...
            const publicationResult = await client.query(`INSERT INTO publications (title, start_date, end_date, status, protocol_version) VALUES ($1, $2, $3, 'DRAFT', $4) RETURNING id`, [title, matchDatetime, new Date(matchDatetime.getTime() + 3 * 24 * 60 * 60 * 1000), PARSER_VERSION]); 
            publicationId = publicationResult.rows[0].id; 
        } 
        const matchResult = await client.query(`INSERT INTO matches (publication_id, league_id, home_team_id, away_team_id, match_datetime, irg_index) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (publication_id, home_team_id, away_team_id, match_datetime) DO NOTHING RETURNING id`, [publicationId, leagueId, homeTeamId, awayTeamId, matchDatetime, parsedData.match_info.irg_index]); 
        if (matchResult.rows.length === 0) { 
            await client.query('ROLLBACK'); 
            return { success: false, code: 'ERR_DUPLICATE_MATCH', message: 'Ce match existe déjà pour cette publication.' }; 
        } 
        const matchId = matchResult.rows[0].id; 
        await client.query(`INSERT INTO match_category_rankings (match_id, publication_id, category_name, rank_in_category) VALUES ($1, $2, $3, $4)`, [matchId, publicationId, parsedData.match_info.category_name, parsedData.match_info.rank_in_category]); 
        ...
        try { 
            await client.query(`INSERT INTO import_logs (user_id, status, raw_text_length, execution_time_ms) VALUES ($1, 'SUCCESS', $2, $3)`, [userId, parsedData.raw_text_length || 0, Date.now() - startTime]); 
        } catch (logErr) { 
        ...
        if (error && error.code === '23505') return { success: false, code: 'ERR_DUPLICATE_MATCH', message: 'Ce match existe déjà pour cette publication.' }; 
