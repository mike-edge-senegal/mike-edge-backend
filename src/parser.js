...
/**
 * Note 8 : Calcule les étoiles à partir de la probabilité numérique.
 * Barème PROBABILITÉ : 85/75/65/55.
 * Retourne null si probabilityPct est absent ou invalide.
 */
function calculateStarsFromProb(probabilityPct) {
    if (probabilityPct === null || probabilityPct === undefined) return null;
...
function parseTelegramText(rawText) {
    ...
    if (pdjSection) {
        const pdjProb = extractPercentage(pdjSection) ?? globalProb;
        ...
        output.pari_du_jour = {
            ...
            probability_pct: pdjProb,
            robustness_pct: extractRobustnessScoreOrText(pdjSection) ?? globalRob ?? output.match_info.irg_index,
            confidence_stars: calculateStarsFromProb(pdjProb)
                ?? extractConfidenceStarsOrText(pdjSection)
                ?? globalStars
                ?? 1
        };
    }
    ...
    if (top5Section) {
        top5Section.split(/(?=^\s*(?:🥇|🥈|🥉|4️⃣|5️⃣|P[1-5]\s*:|\d\.\s*(?:SÉCURITÉ|VALEUR|OPPORTUNITÉ)))/gim).filter(b => /^\s*(?:🥇|🥈|🥉|4️⃣|5️⃣|P\d|P\d\s*:|\d\.)/im.test(b)).slice(0, 5).forEach((block, index) => {
            ...
            const prob = extractPercentage(block);
            ...
            output.top_5_premium.push({
                ...
                confidence_stars: calculateStarsFromProb(prob)
                    ?? extractConfidenceStarsOrText(block)
                    ?? 1
            });
        });
    }
    ...
    if (valuePremiumSection) {
        const prob = extractPercentage(valuePremiumSection);
        ...
        output.value_bet_premium = {
            ...
            confidence_stars: calculateStarsFromProb(prob)
                ?? extractConfidenceStarsOrText(valuePremiumSection)
                ?? 1
        };
    }
    ...
    if (valueSpeculativeSection) {
        const prob = extractPercentage(valueSpeculativeSection);
        ...
        output.value_bet_speculatif = {
            ...
            confidence_stars: calculateStarsFromProb(prob)
                ?? extractConfidenceStarsOrText(valueSpeculativeSection)
                ?? 1
        };
    }
    ...
    if (ticketSection) {
        ...
        output.ticket_combine = {
            ...
            confidence_stars: extractConfidenceStarsOrText(ticketSection) ?? 1,
